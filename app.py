...
from modules.advisor import generate_advice, find_ideal_player, generate_ai_comment
...
.rank-medal-1 { color: #ffd700; font-weight: 700; font-size: 18px; }
.rank-medal-2 { color: #c0c0c0; font-weight: 700; font-size: 16px; }
.rank-medal-3 { color: #cd7f32; font-weight: 700; font-size: 16px; }
.rank-medal-n { color: #5a7a9a; }
/* AIコメントボックス */
.ai-comment-box {
    background: #0d1626;
    border: 1px solid #1e3a5f;
...
with tab_player:
    ...
    if ideal_info:
        ...
        # AIコメント生成
        with st.spinner("AIコーチングコメントを生成中..."):
            ai_result = generate_ai_comment(
                str(selected_player), ideal_info, advice_list, selected_metrics)

        if isinstance(ai_result, dict):
            st.markdown(f"""
            <div class="ai-comment-box">
                <div class="ai-comment-label">AI COACH — 総評</div>
                {ai_result.get("総評", "")}
            </div>
            """, unsafe_allow_html=True)

            improvements = ai_result.get("重点改善", [])
            if improvements:
                items_html = "".join(
                    f'<div style="padding:4px 0;">▸ {item}</div>'
                    for item in improvements
                )
                st.markdown(f"""
                <div class="ai-comment-box">
                    <div class="ai-comment-label">AI COACH — 重点改善ポイント</div>
                    {items_html}
                </div>
                """, unsafe_allow_html=True)

            strength = ai_result.get("強み活用", "")
            if strength:
                st.markdown(f"""
                <div class="ai-comment-box">
                    <div class="ai-comment-label">AI COACH — 強みの活かし方</div>
                    {strength}
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="ai-comment-box">{ai_result}</div>
            """, unsafe_allow_html=True)
...
with tab_ranking:
    ...
    if not ranking_metrics:
        ...
        for _, row in ranking_df.iterrows():
            ...
            # スコアバー
            bar_html = f"""
            <div style="background:#0a0e1a; border-radius:2px; height:4px; width:120px; display:inline-block; vertical-align:middle; margin-left:8px;">
